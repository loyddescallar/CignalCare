...
export default function AdminDashboard() {
  ...
  const kpis = [
    { label: "Total Tickets",   value: tickets.filter(t => t.category !== "Technician Request").length, color: "text-red-600",   bg: "bg-red-50",   icon: <TicketIcon className="h-5 w-5" />,           path: "/admin/tickets" },
    { label: "Total Customers", value: customers.length,    color: "text-blue-600",  bg: "bg-blue-50",  icon: <UsersIcon className="h-5 w-5" />,            path: "/admin/customers" },
    { label: "Tech Requests",   value: techCount,           color: "text-amber-600", bg: "bg-amber-50", icon: <WrenchScrewdriverIcon className="h-5 w-5" />, path: "/admin/technicians" },
    { label: "Total Revenue",   value: `₱${totalRevenue.toLocaleString()}`, color: "text-green-600", bg: "bg-green-50", icon: <CreditCardIcon className="h-5 w-5" />, path: "/admin/transactions" },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <motion.div variants={stagger} initial="hidden" animate="show" className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {kpis.map(s => (
          <motion.div key={s.label} variants={fadeUp}
            ...
            className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 cursor-pointer hover:shadow-md transition-shadow"
          >
      ...
      {/* Middle Row */}
      <motion.div
        initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.3 }}
              ...
              return (
                <div key={label}>
                  ...
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.max((value / total) * 100, value ? 6 : 0)}%` }}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
          ...
          <div className="flex h-32 items-end gap-1.5 border-b border-l border-dashed border-slate-200 pb-2 pl-2">
            {growth.map((point, i) => {
              ...
              const h   = Math.max((point.value / max) * 75, point.value ? 6 : 2);
              return (
      ...
      {/* Bottom Row */}
      <motion.div
        initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.28, duration: 0.3 }}
      ...
      >
        <div className="bg-white rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="divide-y divide-slate-50">
            ...
            : tickets.filter(t => t.category !== "Technician Request").slice(0, 5).map((t, i) => (
              <motion.div key={t.id}
                initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + i * 0.05, duration: 0.25 }}
            ...
            : customers.slice(0, 5).map((c, i) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-red-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">{c.accountName?.[0]?.toUpperCase()}</div>
                  <div>
