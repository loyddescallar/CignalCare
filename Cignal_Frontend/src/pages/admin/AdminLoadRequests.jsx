...
export default function AdminLoadRequests() {
  ...
  const load = async () => {
    setLoading(true);
    try { const res = await getAllLoadRequests(); setRequests(res.data?.requests || res.data || []); }
  ...
  const filtered = useMemo(() => {
    ...
    return requests
      ...
      .filter(r => r.account_name?.toLowerCase().includes(q) || r.account_number?.toLowerCase().includes(q) || r.reference_no?.toLowerCase().includes(q));
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.3 }}
        ...
        <div className="flex flex-wrap items-center gap-3 mb-3">
          <div className="flex items-center gap-2 bg-slate-100 rounded-xl px-3 py-2 flex-1 max-w-xs">
            ...
            <input type="text" placeholder="Search name, account, reference..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead><tr className="border-b border-slate-100 bg-slate-50">{["#","Account Name","Account No.","Plan","Amount","Payment","Reference","Status","Date","Actions"].map(h => (<th key={h} className="text-left py-2.5 px-3 text-slate-500 font-semibold uppercase tracking-wide" style={{ fontSize: "10px" }}>{h}</th>))}</tr></thead>
            <tbody>
              {loading ? <tr><td colSpan={10} className="py-10 text-center text-slate-400">Loading...</td></tr>
              : filtered.length === 0 ? <tr><td colSpan={10} className="py-10 text-center text-slate-400">No load requests found.</td></tr>
              : filtered.map((r, i) => {
                ...
                return (
                  <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03, duration: 0.2 }}
                    ...
                    <td className="py-2 px-3"><button onClick={() => openModal(r)} className="text-xs px-3 py-1 bg-red-600 text-white rounded-xl hover:bg-red-700 font-semibold">Review</button></td>
                  </motion.tr>
                );
              })}
            </tbody>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.18 }}
            ...
            <div className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                {[{label:"Account Name",value:selected.account_name},{label:"Account No.",value:selected.account_number},{label:"Plan",value:selected.plan_name},{label:"Amount",value:`₱${Number(selected.amount||0).toLocaleString()}`},{label:"Payment",value:selected.payment_method},{label:"Reference No.",value:selected.reference_no}].map(f=>(
                  <div key={f.label} className="bg-slate-50 rounded-xl p-3"><p className="text-slate-400 font-semibold" style={{fontSize:"10px"}}>{f.label}</p><p className="text-xs font-semibold text-slate-800 mt-1">{f.value}</p></div>
                ))}
              </div>
              {selected.receipt_photo && (
                <div><p className="text-xs text-slate-500 font-medium mb-1" style={{fontSize:"10px"}}>RECEIPT PHOTO</p>
                  <img src={selected.receipt_photo} alt="Receipt" className="w-full max-h-48 object-contain rounded-xl border border-slate-200" /></div>
              )}
              <div><label className="block text-xs text-slate-500 font-medium mb-1" style={{fontSize:"10px"}}>Update Status</label>
              ...
              <div><label className="block text-xs text-slate-500 font-medium mb-1" style={{fontSize:"10px"}}>Admin Note</label>
                <textarea value={adminNote} onChange={e => setAdminNote(e.target.value)} rows={2} placeholder="Optional note..." className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs outline-none focus:border-red-500 resize-none" />
              </div>
