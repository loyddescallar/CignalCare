...
import { Search, MessageSquare, Trash2, ChevronDown, X, Filter } from "lucide-react";
...
export default function AdminTickets() {
  ...
  }, [supportTickets, search, statusFilter, catFilter]);
  ...
  const handleDelete = async () => {
    ...
    try {
      await ticketApi.deleteTicket(deleteTarget.id);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
        <div className="flex flex-wrap items-center gap-3 mb-3">
          <div className="flex items-center gap-2 bg-slate-100 rounded-xl px-3 py-2 flex-1 max-w-xs">
            ...
            <input type="text" placeholder="Search subject, ID, account..." value={search}
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {["ID","Account","Subject","Category","Priority","Status","Date","Actions"].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="py-10 text-center text-slate-400">Loading tickets...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={8} className="py-10 text-center text-slate-400">No tickets found.</td></tr>
              ) : filtered.map(t => {
                const s = statusCfg[t.status] || statusCfg["Open"];
                return (
                  <tr key={t.id} className="border-b border-slate-50 hover:bg-slate-50 last:border-0">
                    <td className="py-2 px-3 font-mono text-slate-400">#{t.id}</td>
                    <td className="py-2 px-3">
                      <p className="font-semibold text-slate-800">{t.accountName || "—"}</p>
                      <p className="text-slate-400 font-mono" style={{ fontSize: "10px" }}>{t.accountNumber || ""}</p>
                    </td>
                    <td className="py-2 px-3 font-semibold text-slate-800 max-w-[150px] truncate">{t.subject}</td>
                    <td className="py-2 px-3 text-slate-500">{t.category}</td>
                    <td className="py-2 px-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${priorityCfg[t.priority] || "bg-slate-100 text-slate-600"}`}>{t.priority}</span>
                    </td>
                    <td className="py-2 px-3">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-5">
              <p className="text-xs text-slate-600 leading-relaxed">Delete ticket <span className="font-semibold">#{deleteTarget.id}</span> — "{deleteTarget.subject}"? This cannot be undone.</p>
