...
import { TrendingUp, Users, CreditCard, TicketIcon } from "lucide-react";
...
export default function AdminAnalytics() {
  ...
  const BarChart = ({ data, color = "bg-red-500" }) => {
          ...
          <div key={d.key} className="flex flex-1 flex-col items-center gap-1">
            ...
            <div className="flex h-20 items-end w-full">
              <div className={`w-full rounded-t-lg ${color}`}
                style={{ height: `${Math.max((d.value / max) * 75, d.value ? 4 : 2)}px` }} />
            </div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: "Total Tickets",    value: tickets.length,                       color: "text-red-600",    icon: <TicketIcon size={15} className="text-red-500" /> },
