...
const ticketStatusConfig = { Open: { classes: "bg-red-100 text-red-700", Icon: Flag }, "In Progress": { classes: "bg-amber-100 text-amber-700", Icon: Clock }, Resolved: { classes: "bg-green-100 text-green-700", Icon: CheckCircle2 }, Closed: { classes: "bg-slate-100 text-slate-600", Icon: CheckCircle2 } };
...
export default function AdminCustomerProfile() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const custRes = await customerApi.getCustomerById(id); const cust = custRes.data?.customer; setCustomer(cust);
        ...
        const allTickets = ticketRes.data?.tickets || []; const allTech = techRes.data?.requests || techRes.data?.technicians || []; const allTx = txRes.data?.history || [];
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-slate-400"><div className="text-center"><div className="w-8 h-8 border-2 border-red-600 border-t-transparent rounded-full animate-spin mx-auto mb-3" /><p className="text-sm">Loading profile...</p></div></div>;
  if (!customer) return <div className="flex flex-col items-center justify-center h-64 text-slate-400"><User size={32} className="mb-3 opacity-30" /><p className="text-sm">Customer not found.</p><button onClick={() => navigate("/admin/customers")} className="mt-3 text-xs text-red-600 hover:underline">← Back</button></div>;
  const actStatus = getActivityStatus(customer.lastLoadDate); const activeTickets = tickets.filter(t => !["Resolved","Closed"].includes(t.status)).length; const openTechReqs = techReqs.filter(r => !["Completed","Cancelled"].includes(r.status)).length; const churn = computeChurnScore(customer.lastLoadDate, activeTickets, openTechReqs); const totalSpent = transactions.reduce((s, t) => s + Number(t.loadAmount || t.amount || 0), 0);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: "Total Tickets",    value: tickets.length,    sub: `${activeTickets} active`,  color: "text-slate-800", icon: null },
        ...
        <div className="lg:col-span-2 space-y-3">
          <div className="flex gap-1 bg-slate-100 rounded-xl p-1 w-fit">
            {([{key:"transactions",label:"Transactions",Icon:CreditCard,count:transactions.length},{key:"tickets",label:"Tickets",Icon:Flag,count:tickets.length},{key:"service",label:"Service",Icon:Wrench,count:techReqs.length}]).map(tab=>(
              <button key={tab.key} onClick={()=>setActiveTab(tab.key)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-colors ${activeTab===tab.key?"bg-white text-red-600 shadow-sm":"text-slate-500 hover:text-slate-700"}`}>
                <tab.Icon size={12}/>{tab.label}<span className={`px-1.5 py-0.5 rounded-full font-bold ${activeTab===tab.key?"bg-red-100 text-red-600":"bg-slate-200 text-slate-500"}`} style={{fontSize:"10px"}}>{tab.count}</span>
              </button>
            ))}
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-slate-200">
            ...
            {activeTab==="transactions"&&(transactions.length===0?<div className="py-10 text-center text-xs text-slate-400">No transactions.</div>:<div className="divide-y divide-slate-50">{transactions.map((tx,i)=>(<div key={i} className="px-4 py-3 hover:bg-slate-50 flex items-center justify-between"><div><p className="text-xs font-semibold text-slate-800">₱{Number(tx.loadAmount||tx.amount||0).toLocaleString()}</p><p className="text-xs text-slate-400 mt-0.5">{tx.description||"Prepaid Load"}</p><p className="text-xs text-slate-400 mt-0.5">{(tx.created_at||"").split(" ")[0]}</p></div><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${tx.status==="completed"?"bg-green-50 text-green-600":"bg-amber-50 text-amber-600"}`}>{tx.status||"completed"}</span></div>))}</div>)}
