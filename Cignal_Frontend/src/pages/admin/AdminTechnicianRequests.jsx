...
export default function AdminTechnicianRequests() {
  ...
  const kpis = [
    ...
    { label: "Cancelled", value: requests.filter(r => r.status === "Cancelled").length, dot: "bg-slate-400", color: "text-slate-500" },
  ];
            ...
            {["All", ...LOCATIONS].map(loc => (
              <button key={loc} onClick={() => setLoc(loc)}
                className={`text-xs px-2.5 py-1.5 rounded-xl font-medium transition-colors ${
                  locFilter === loc ? "bg-red-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                <MapPin size={9} className="inline mr-0.5" />{loc}
              </button>
            ))}
          </div>
          <div className="flex gap-1.5 flex-wrap">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {["#","Issue","Account No.","Contact","Location","Preferred Date","Technician","Status","Actions"].map(h => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="py-10 text-center text-slate-400">No requests found.</td></tr>
              ) : filtered.map(r => {
                ...
                return (
                  <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50 last:border-0">
                    ...
                    <td className="py-2 px-3 font-semibold text-slate-800 max-w-[140px] truncate">{r.issueDescription}</td>
                    ...
                    <td className="py-2 px-3">
                      <button onClick={() => openModal(r)} className="text-xs px-3 py-1 bg-red-600 text-white rounded-xl hover:bg-red-700 font-semibold">Manage</button>
                    </td>
                  </tr>
                );
              })}
          ...
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              <div className="bg-slate-50 rounded-xl p-3 space-y-1.5">
                ...
                {selected.location && <div className="flex items-center gap-2"><MapPin size={12} className="text-slate-400" /><p className="text-xs text-slate-600">{selected.location}</p></div>}
